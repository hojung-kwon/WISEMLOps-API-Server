...
class ModelVersionService:
    ...
    def delete_model_version(self, name: str, version: str):
        try:
            self._mlflow_client.delete_model_version(name, version)
            return name
        except MlflowException as e:
            raise MlflowApiError(e)
    ...
    def delete_model_version_tag(self, name: str, version: Optional[str] = None, key: Optional[str] = None,
        ...
        try:
            self._mlflow_client.delete_model_version_tag(name, version=version, key=key, stage=stage)
            return name
        except MlflowException as e:
            raise MlflowApiError(e)
    ...
    def set_model_version_tag(self, name: str, version: Optional[str] = None, key: Optional[str] = None,
        ...
        try:
            self._mlflow_client.set_model_version_tag(name, version=version, key=key, value=value, stage=stage)
            return name
        except MlflowException as e:
            raise MlflowApiError(e)
